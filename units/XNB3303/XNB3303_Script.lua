...
--            LOG('bone = '..repr(bone))
        ...
        self.ElevatorManip = CreateAnimator(self):PlayAnim('/units/XNB3303/XNB3303_Elevator.sca' ):SetRate(1 * self.ElevatorRate)
    ...
    CalcReloadAnimParams = function(self)
        ...
        if MuzzleDelay > 0 then
            ...
            elseif FreeTime < 0 then
                WARN('XNB3303: MuzzleDelay defined in blueprint is to short or the pitch speed is to slow')
            end
        end
        ...
        if self.ReloadWaitTime <= 0 then
            WARN('XNB3303: need more time for the reload animation, decrease the rate of fire.')
            WARN('XNB3303: Interval = '..repr(Interval)..' SalvoTime = '..repr(SalvoTime)..' ElevatorTime = '..repr(ElevatorTime))
            WARN('XNB3303: RotatorTime = '..repr(RotatorTime)..' GoHorzTime = '..repr(GoHorzTime)..' ReloadWaitTime = '..repr(self.ReloadWaitTime))
        end
        if math.ceil( bpw.EnergyRequired / bpw.EnergyDrainPerSecond ) > Interval then
            WARN('XNB3303: Energy consumption prevents reaching the ROF. Energy: '..repr(math.ceil( bpw.EnergyRequired / bpw.EnergyDrainPerSecond ))..' seconds, ROF '..repr(Interval)..' seconds.')
        end
