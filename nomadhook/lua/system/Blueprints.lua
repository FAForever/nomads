...
function ModBlueprints(all_bps)
    ...
    for _, bp in all_bps.Unit do
        ...
        # Units that have build presets get some additional BP data, to make that feature work.
        if USEBUILDPRESETS then
...
# TODO: This is an attempt to allow 'metal' worlds as found in total annihilation. On those worlds you can build metal extractors
# everywhere on the map. The code below should enable such behaviour in FA. The 'only' thing left to do is find a way to make the
# map indicate to this bit of code that it wants this behaviour. So far no luck. Either fix and finish or delete this code (maybe
# there's a better way).

#LOG('debug = '..repr(debug.getinfo()))
#local sceninfo = __G
#LOG('SessionGetScenarioInfo = '..repr())

    local AllowEconomicStructuresEverywhere = false

    if AllowEconomicStructuresEverywhere then
        # This allows building mexes and hydro plants everywhere on the map. The structures produce 95% less though!
        local econScale = 0.05
        for id, bp in all_bps.Unit do
            if bp.Physics.BuildRestriction and bp.Physics.BuildRestriction != '' then
            ...
            if table.find( bp.Categories, 'MASSEXTRACTION') then
