...
float4 CartographicPS( CARTOGRAPHIC_VERTEX vertex, uniform bool hypsometric) : COLOR0
{
	...
	if ( hypsometric )
	{
		clip(tex2D(albedoSampler,vertex.uv).a-0.5);
		color = tex1D(hypsometricSampler,elevation).rgb;
	}
	
	float  ca = 0.25;
	float cd0 = ( vertex.tone > 0.21 ) ? 0.25 : 0.0;
	float cd1 = ( vertex.tone > 0.66 ) ? 0.50 : 0.0;
	float  cs = ( vertex.tone > 0.95 ) ? 0.10 : 0.0;
...
///
///
float4 CartographicShieldPS0( CARTOGRAPHIC_VERTEX vertex) : COLOR0
{
    return float4(1,1,1,0.5 * vertex.tone);
}
float4 CartographicShieldPS1( CARTOGRAPHIC_VERTEX vertex) : COLOR0
{
			   ...
			   uniform int alphaRef ) : COLOR0
{
    ...
    color.rgb = ComputeFogColor(color.rgb,vertex.fog);
								    ...
								    uniform int alphaRef ) : COLOR0
{
    ...
    float3 color = ComputeFogColor(vertex.color.rgb * albedo.rgb * light,vertex.fog);
								  ...
								  uniform int alphaRef ) : COLOR0
{
    ...
    float3 color = ComputeFogColor(2 * light.rgb * light.rgb * albedo.rgb,vertex.fog);
					   ...
					   uniform int alphaRef ) : COLOR0
{
    ...
    color.rgb = ComputeFogColor(color.rgb,vertex.depth.zw);
...
float4 MapImagerPS0( NORMALMAPPED_VERTEX vertex) : COLOR0
{
    float3x3 rotationMatrix = float3x3(vertex.binormal,vertex.tangent,vertex.normal);
    float3 normal = ComputeNormal(normalsSampler,vertex.texcoord0.zw,rotationMatrix);
    float dotLightNormal = saturate(dot(sunDirection,normal));

    float4 albedo = tex2D(albedoSampler,vertex.texcoord0.xy);
    albedo.rgb = dotLightNormal * albedo.rgb * vertex.color.rgb;
								...
								uniform int alphaRef ) : COLOR0
{
	...
	float emissive = glowMultiplier * specular.b;
	float3 color = albedo.rgb * ( emissive.r + light + phongMultiplicative) + phongAdditive;
    color = ComputeFogColor(color,vertex.depth.zw);
...
float4 WreckagePS_LowFidelity(VERTEXNORMAL_VERTEX vertex) : COLOR0
{
    ...
    color = ComputeFogColor( color,vertex.fog);
    return float4(color.rgb,vertex.material.g);
}
...
float4 NormalMappedTerrainPS( NORMALMAPPED_VERTEX vertex ) : COLOR
{
    ...
    float3 color = ComputeFogColor(light * albedo.rgb,vertex.depth.zw);
...
float4 CommandFeedbackPS( VERTEXNORMAL_VERTEX vertex, 
						  uniform bool fade ) : COLOR0
{
    float4 color = tex2D( albedoSampler, vertex.texcoord0.xy);
    color.rgb = ComputeFogColor(color.rgb,vertex.fog);
...
float4 AeonCZARPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    ...
    color += float3(0,1,1) * (secondary.b + secondary2.g )* (1-albedo.a);
...
float4 UnitFalloffPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
	...
	// Lerp in team color in masked areas
  	diffuse.rgb = lerp( vertex.color.rgb, diffuse.rgb, 1 - specular.a );

	// Calculate lookup into falloff ramp
	float NdotV = pow(1 - saturate(dot( normalize(vertex.viewDirection), normal )), 0.6);
	float4 fallOff = tex2D( falloffSampler, float2(  NdotV, 0.00 ) );  	
	...
	float3 phongAdditive = pow( specularAmount, 9) * specular.g;
	...
	float3 color = diffuse.rgb * ((glowMultiplier * specular.b) + light);
...
float4 UEFBuildHiFiPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    ...
    float3 current = lerp( color + secondary, float3( 0, 0, 1.0 ), min(max(frac( 0.02 * time), 0.35), 0.7) );
    outColor =  lerp( current, color, vertex.material.y);
...
float4 NormalMappedInsectPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
	...
	float alpha = mirrored ? 0.5 : specular.b + phongAdditive.a + glowMinimum;
...
>
{
    pass P0
    {
        ...
        VertexShader = compile vs_1_1 CartographicVS();
        PixelShader = compile ps_2_0 CartographicShieldPS0();
    }
    pass P1
    {
		AlphaState( AlphaBlend_Default_Write_A )
		RasterizerState( Rasterizer_Cull_CW )
		DepthState( Depth_Enable )

        VertexShader = compile vs_1_1 CartographicVS();
        PixelShader = compile ps_2_0 CartographicShieldPS1();
    }
}
...
<
    ...
    int renderStage = STAGE_DEPTH + STAGE_POSTWATER + STAGE_POSTEFFECT;
    int parameter = PARAM_FRACTIONCOMPLETE;
>
{
    pass P0
...
>
{
    pass P0
    {
        ...
        PixelShader = compile ps_2_0 CommandFeedbackPS(true);
    }
}
...
>
{
    pass P0
    {
        ...
        VertexShader = compile vs_1_1 VertexNormalVS();
		PixelShader = compile ps_2_0 CommandFeedbackPS(false);
    }
}
